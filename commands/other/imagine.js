...
export default {
  ...
  cooldowns: 50,
  description: "فم بتوليد صور بإستخدام الذكاء الإصطناعي DALL·E",
  role: "member",
  aliases: ["dalle", "تخيل"],
  execute: async ({ api, event, args, Economy }) => {
    ...
    // استخراج النص المُدخل وترجمة النص
    ...
    try {
      ...
      if (!data || data.length === 0) {
        return api.sendMessage("⚠️ | حدث خطأ أثناء عملية توليد الصورة", event.threadID, event.messageID);
      }

      // تحميل الصورة وحفظها في مجلد cache
      ...
      // جلب الوقت والتاريخ باستخدام moment-timezone
      ...
      api.sendMessage("⚠️ | حدث خطأ أثناء توليد الصورة، الرجاء المحاولة مرة أخرى.", event.threadID, event.messageID);
    }
  }
};
