...
export default {
    ...
    try {
      ...
      api.sendMessage({ body: msg, attachment: attachments }, event.threadID, (error, info) => {
        ...
        global.client.handler.reply.set(info.messageID, {
          ...
          unsend: false // عدم حذف الرسالة في حالة الرد الخاطئ أو الخطأ
        });
  ...
  async onReply({ api, event, reply }) {
    ...
    try {
      const downloadUrl = `https://api-improve-production.up.railway.app/yt/download?url=${encodeURIComponent(videoUrl)}&format=mp3`;
      const downloadResponse = await axios.get(downloadUrl);

      const audioFileUrl = downloadResponse.data.audio;
