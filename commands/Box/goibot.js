...
export default {
  ...
  events: async ({ api, event, Users, Threads, Economy }) => {
    ...
    const responses = [
      ...
      { keywords: ["تزوجيني يا كاغويا", ""], response: "️في أحلامك" },
      ...
      { keywords: ["السلام عليكم", "سلام"], response: "️و؏ٌٍـلًِيٌِگِـٍٍّّـًـًٍ(🌹)ٌٍـٌٍـًٌٍم السـ͜(🤝)ـلاﺂ͘م وݛحـٍّْـٍّْ⁽😘₎ـٍّْمهہ الًـًٍٍۖـٍۖ(☝)ٍۖـًٍٍٍّـًٍلۖهًٍۖۂ وبـۗـۗـۗـۗـۗـۗركۧۧـ(ۗ😇)ـۗـۗاتهۂ" },
      { keywords: ["وداعا", "أنا ذاهب"], response: "️وداعا مع السلامه آمل أن نراك قريبا ☺️" },
      { keywords: ["من أنتي يا هيناتا", "عرفينا على نفسك"], response: "️حسنا إسمي كاغويا تشرفت بمعرفتك..." },
      { keywords: ["بوت أحمق"], response: "️من قال لك إستخدمه في المقام الأول 😒" },
    ...
    // تحقق من وجود تطابق، ووقف التحقق بعد الرد الأول
    for (const item of responses) {
      for (const keyword of item.keywords) {
        if (message.includes(keyword)) {
          api.sendMessage(item.response, threadID);
          return;  // هنا نتوقف بعد الرد الأول لتجنب السبام
        }
      }
    }
  },
